...
class App extends Component {
  ...
  checkImage = event => {
    const {randomThumbnail} = this.state
    if (randomThumbnail.thumbnailUrl === event.target.src) {
      this.setState(prevState => ({
  ...
  render() {
    ...
    const changeUi = activeState || startTime === 0
      ...
      <>
        <nav className="navbar">
          ...
          <div className="emoji-logo">
            <p className="para">Score:{scoreCount}</p>
            <div className="timer-container">
              ...
              />
              <p className="para">{startTime} sec</p>
            </div>
          </div>
        </nav>
        {changeUi ? (
          ...
          <div className="main-container">
            ...
            <ul className="tabs-container">
              {tabsList.map(tabDetails => (
                <li className="tab-item-container" key={tabDetails.tabId}>
                  <button
                    type="button"
                    className={
                      tabDetails.tabId === activeTabId
                        ? 'tab-btn btn'
                        : 'tab-btn'
                    }
                    onClick={this.onChangeTab}
                    value={tabDetails.tabId}
                  >
